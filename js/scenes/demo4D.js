...
export const init = async model => {
   ...
   inputEvents.onPress = hand => {
      p = inputEvents.pos(hand);
      isDragging = Math.max(Math.abs(p[0]-center[0]),
                            Math.abs(p[1]-center[1]),
                            Math.abs(p[2]-center[2])) < 1.8 * size;
   }

   inputEvents.onDrag = hand => {
      if (isDragging) {
         let q = inputEvents.pos(hand);
         let d = cg.subtract(p, q);
         for (let i = 0 ; i < 3 ; i++)
            rot(40 * size * d[i], i, 3);
         p = q;
         server.broadcastGlobal('rot4D');
      }
   }
   ...
   model.animate(() => {
      rot4D = server.synchronize('rot4D');
