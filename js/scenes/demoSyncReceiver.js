...
export const init = async model => {
   ...
   let button1Box = uiBox('button', model,
      () => {
       ...
       () => {
           server.get((15 * slider1Value >> 0).toString(), (value) => {
               let str = value.split("+");
               localStorage.setItem("BoundInfo", str[0]);
               localStorage.setItem("OriginCoords", str[1]);
           });

           button1State = 'received';
       },
   );
