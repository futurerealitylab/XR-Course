...
export const init = async model => {
   ...
   let animal = npcSystem.addNPC("tetrapod", "animal");
   ...
   model.animate(() => {
      ...
      let lookVec = leftHitPosLocal 
               ? [leftHitPosLocal[0]-animal.center[0], 0, 
                  leftHitPosLocal[2]-animal.center[2]] : null;
      npcSystem.setNPCMoveVec("animal", moveVec);
      npcSystem.setNPCLookVec("animal", lookVec);


      targetFootL.setMatrix(n_rootNPCs.getGlobalMatrix()).move(animal.targetPosFootL).scale(.2,.2,.2);
      targetFootR.setMatrix(n_rootNPCs.getGlobalMatrix()).move(animal.targetPosFootR).scale(.2,.2,.2);
