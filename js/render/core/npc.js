...
export class NPCSystem {
   ...
   constructor(model, dimX = 16, dimZ = 16) {
      this.r_model = model;
      this.terrain = new NPCTerrain(dimX, dimZ);
   }
   ...
   update() {
      Object.entries(this.dictNPC).forEach(([id, NPCObj]) => {
         ...
         NPCObj.update(this.r_model.time, this.r_model.deltaTime, moveVec);
         NPCObj.render();
      })
   }
...
export class NPCTerrain {
   ...
   generateMesh(meshName = "_NPCTerrain") {
      ...
      for (let z = 0; z < this.dimZ; ++z) {
            let y = this.grid[x][z];
            let matrix = cg.mMultiply(cg.mTranslate(x+.5, y/2, z+.5), 
...
export class NPC {
   ...
   update(time, delta, movVec = [0,0,0]) {}
   initRender(node) {
...
export class RobotMultiLegs extends NPC {
   ...
   update(time, delta, movVec, probe) {
      this.movVec = movVec;
